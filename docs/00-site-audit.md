...
公开 Release 实际包含 `XianYunAIVoiceInput_0.1.5_x64-setup.exe`。但是本任务明确要求官网 V1 不得表述“Windows 已经可用”，因此官网 manifest 与页面仍将 Windows 标为 `coming-soon`，不提供 Windows 下载按钮。该产品策略与发布资产现状存在差异，需要后续人工决定何时开放官网入口。
...
发布边界：官网可提供 macOS Apple Silicon `0.1.5` Beta 下载；Windows 必须按本轮要求继续显示“开发中”；真实产品截图暂以明确占位替代；隐私页对未核实细节标注“待产品实现确认”。

## 9. 2026-07-20 截图补充记录

V1 发布后，已使用 macOS 0.1.5 安装版真机补充三张公开素材：首页运行准备、脱敏历史筛选、本地 SenseVoiceSmall 状态。历史页只显示本轮公开测试句；三张图片均未显示 API Key、个人姓名、单位信息、私有文件路径或仓库信息。官网占位已替换为 AVIF + JPEG 响应式图片。
